...
import { PageHeader, Card, CardHeader, KpiCard, StatusBadge } from "@/components/ui-kit/Primitives";
...
const tone = (s: string) => ({ Captured: "success", Pending: "warning", Refunded: "error", Failed: "error" }[s] as any) || "neutral";
            ...
            <tbody>
              {txns.map((t) => (
                <tr key={t.id} className="border-b border-border-subtle hover:bg-surface-2/50">
                  ...
                  <td className="px-4 py-3"><StatusBadge tone={tone(t.status)}>{t.status}</StatusBadge></td>
