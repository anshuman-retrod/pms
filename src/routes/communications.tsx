...
function ComsPage() {
  return (
    <div>
      <PageHeader eyebrow="Guests" title="Guest Communications" actions={<Button size="sm"><Plus className="h-3.5 w-3.5" />New message</Button>} />
      ...
      <div className="grid grid-cols-1 gap-6 p-6 lg:grid-cols-[320px_1fr]">
        <Card>
          ...
          <ul className="divide-y divide-border-subtle">
            {threads.map((t, i) => (
              <li key={t.name} className={`flex cursor-pointer items-start gap-3 px-4 py-3 hover:bg-surface-2/60 ${i === 1 ? "bg-primary-tint/30" : ""}`}>
                <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-foreground text-[11px] font-semibold text-background">{t.name.split(" ").map(s=>s[0]).slice(0,2).join("")}</div>
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-1 flex items-center gap-2">
                    ...
                    {t.unread > 0 && <span className="rounded-sm bg-primary px-1.5 py-px text-[10px] font-medium text-primary-foreground">{t.unread}</span>}
                  </div>
                </div>
              </li>
            ))}
        ...
        <Card className="flex flex-col">
          ...
          <div className="border-t border-border-subtle p-3">
            <div className="flex items-center gap-2 rounded-md border border-border bg-surface px-3 py-2 focus-within:border-primary">
              <input className="flex-1 bg-transparent text-[13px] outline-none placeholder:text-text-disabled" placeholder="Type a message…" />
...
function Bubble({ from, text, time }: { from: "me" | "them"; text: string; time: string }) {
  return (
    <div className={`flex ${from === "me" ? "justify-end" : ""}`}>
      <div className={`max-w-[75%] rounded-lg px-3 py-2 text-[13px] ${from === "me" ? "bg-primary text-primary-foreground" : "bg-surface-2 text-text-primary"}`}>
        <div>{text}</div>
        <div className={`mt-1 text-right text-[10px] ${from === "me" ? "text-primary-foreground/70" : "text-text-disabled"}`}>{time}</div>
      </div>
    </div>
  );
}
