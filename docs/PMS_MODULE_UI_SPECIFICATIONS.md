...
Example: *No work orders yet. Log issues to track repairs and free up rooms.* **[Create work order]**
...
*All new UI must import from `@/components/ui/Primitives` and use design tokens from `src/styles.css`. Refer to `.cursor/rules/retrod-pms-theme.mdc` for colors and typography.*
