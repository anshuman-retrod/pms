          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-[320px_1fr]">
            ...
            <Card>
              ...
              <ul className="max-h-[640px] divide-y divide-border-subtle overflow-y-auto scrollbar-thin">
                {arrivalsToday
                  .concat(arrivalsToday)
                  .slice(0, 8)
                  .map((r, i) => (
                    <li
                      key={i}
                      className={`flex items-center gap-3 px-4 py-3 hover:bg-surface-2/60 cursor-pointer transition ${
                        i === 0 ? "bg-primary-tint/40" : ""
                      }`}
                    >
                      {r.guest.includes("Sophie") || r.guest.includes("Tanaka") ? (
                        <Crown className="h-3.5 w-3.5 text-[var(--color-gold,#c9a84c)]" />
                      ) : (
              ...
              <table className="w-full text-[13px]">
                <thead className="bg-surface-2/40 text-left">
                  <tr className="border-b border-border bg-surface-2/40 text-left">
                    {["Guest", "Room", "Requested time", "Charge", "HK ready", "Decision"].map((h) => (
                  ...
                  {[
                    ...
                    <tr key={i} className="border-t border-border-subtle">
                      ...
                      <td className="px-4 py-3">
                        <StatusBadge tone={x.ok ? "success" : "warning"}>{x.ok ? "Ready" : "Tight"}</StatusBadge>
                      </td>
                      <td className="px-4 py-3">
    ...
    <div>
      <div className="mb-3 flex items-center justify-between">
        <div className="text-[12px] text-text-secondary">
          Pre-assigned: <span className="font-mono text-text-primary">204 · Deluxe King · Floor 2</span>
        </div>
        <Button size="sm" variant="outline">
          Suggest upgrade
        </Button>
        ...
        {ready.map((n) => (
          <button
            ...
            className={`aspect-[3/2.4] rounded-md border-2 p-2 text-left transition ${
              selected === n ? "border-primary bg-primary-tint" : "border-[var(--color-success)] bg-[oklch(0.96_0.04_152)]"
            }`}
          >
            <div
