...
import { cloneRatePlan, createEmptyRatePlan, summarizePlanChanges } from "@/features/rate-plans/lib/form";
import {
  pushRatePlanToSu,
  queueRatePlanSuSync,
...
function mergePlanList(plans: RatePlan[], updated: RatePlan, isNew: boolean): RatePlan[] {
  const base = isNew ? [...plans, updated] : plans.map((item) => (item.id === updated.id ? updated : item));
...
export function RatePlansFeature() {
  ...
  const finalizeSuSync = (plans: RatePlan[], planId: string, jobId: string, actionLabel: string) => {
  ...
  const handleClone = (plan: RatePlan) => {
    ...
    logAuditEvent("Rate plan cloned", plan.externalRatePlanCode, `Cloned to ${copy.externalRatePlanCode}.`);
    openEdit(copy);
    ...
    >
      <div>
        ...
        <div className="space-y-6 p-6">
          ...
          <Card>
            <CardHeader title="Rate plan catalog" hint={`${filtered.length} of ${ratePlans.length} plans`} />
            <div className="flex flex-wrap items-center gap-2 border-b border-border px-5 pb-4">
              ...
              <Button variant="outline" size="sm" onClick={() => toast.info("Import CSV — Phase 3.")}>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[960px] text-[13px]">
                ...
                <tbody>
                  {filtered.map((plan) => (
                    <tr key={plan.id} className="border-b border-border-subtle hover:bg-surface-2/50">
                      ...
                      <td className="px-4 py-3 text-[12px] text-text-secondary">{formatEffective(plan)}</td>
                      <td className="px-4 py-3">
          ...
          {syncJobs.length > 0 && (
            <Card>
              <CardHeader
                title="SU sync jobs"
                hint="Recent rate plan pushes to channel manager"
              />
              <div className="overflow-x-auto">
