...
export function RetrodOneFeature() {
  ...
  const quickActions = useMemo(
    () => filterQuickActions(QUICK_ACTIONS, can, user),
    [can, user],
  );
        ...
        <section className="relative overflow-hidden rounded-2xl border border-border bg-surface p-6 shadow-e2 sm:p-8">
          ...
          <div className="relative flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            ...
            <div className="grid w-full grid-cols-2 gap-3 sm:grid-cols-4 lg:max-w-3xl">
              <KpiCard label="Properties" value={String(PLATFORM_STATS.properties)} accent="brand" />
              <KpiCard
                label="Active Users"
              ...
              <KpiCard
                label="Revenue Today"
                value={PLATFORM_STATS.revenueToday}
                accent="warning"
              />
            </div>
          </div>
              ...
              <ul className="divide-y divide-border-subtle">
                {RECENT_ACTIVITY.map((item) => (
                  <li key={item.id} className="flex gap-4 px-5 py-4">
                    ...
                    <time className="shrink-0 text-[11px] text-text-disabled">{item.timestamp}</time>
                  </li>
                ))}
              </ul>
            </Card>
