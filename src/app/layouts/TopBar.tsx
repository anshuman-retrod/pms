...
const labels: Record<string, string> = {
  "": "Dashboard",
  "dashboard/multi-property": "Multi-Property",
  "front-desk": "Front Desk",
  reservations: "Reservations",
  "check-in": "Check-In / Out",
  housekeeping: "Housekeeping",
  "housekeeping/mobile": "HK Mobile",
  ...
  "channel-manager/connections": "OTA Connections",
  "channel-manager/room-mapping": "Room Mapping",
  "channel-manager/rate-plans": "Rate Plan Mapping",
  "channel-manager/inventory": "Inventory",
  "channel-manager/availability": "Availability Calendar",
  "channel-manager/rates": "Rate Calendar",
  "channel-manager/reservations": "Reservation Sync",
  "channel-manager/sync-logs": "Sync Logs",
  "channel-manager/revenue": "OTA Revenue",
  "channel-manager/property-content": "Property Content",
  "channel-manager/room-content": "Room Content",
  "channel-manager/images": "Image Management",
  "channel-manager/restrictions": "Restrictions",
  "channel-manager/multi-property": "Multi-Property",
  "channel-manager/analytics": "Channel Analytics",
...
const groupOf: Record<string, string> = {
  ...
  feedback: "Guests",
  "registration-cards": "Guests",
  billing: "Commercial",
  payments: "Commercial",
  revenue: "Commercial",
  pricing: "Commercial",
  ota: "Commercial",
  "channel-manager": "Commercial",
  "booking-engine": "Commercial",
  groups: "Commercial",
  corporate: "Commercial",
  packages: "Commercial",
  "add-ons": "Commercial",
  concierge: "Commercial",
...
export function TopBar() {
  ...
  const seg = pathKey.split("/")[0] ?? "";
  const page =
    labels[pathKey] ??
    (pathname === "/analytics/executive" ? labels.analytics : labels[seg]) ??
    "Dashboard";
  const group = groupOf[seg] ?? "Operations";
  const { user } = useAuth();
  const linkTo = pathname.startsWith("/") ? pathname : `/${pathKey}`;
  ...
  return (
    <header className="sticky top-0 z-20 flex h-14 items-center gap-4 border-b border-border bg-surface px-6">
      ...
      <div className="ml-auto flex items-center gap-3">
        <button
          type="button"
          className="hidden lg:flex items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-[13px] font-medium text-primary-foreground shadow-e1 transition hover:bg-primary-pressed"
        >
        ...
        <button
          type="button"
          className="relative rounded-md p-2 text-text-secondary hover:bg-surface-2 hover:text-text-primary"
        >
