...
export function validateRatePlan(
  ...
  const duplicate = ctx.allPlans.some(
    (item) => item.id !== plan.id && item.externalRatePlanCode.trim().toUpperCase() === code.toUpperCase(),
  );
  if (!code) {
    errors.push(issue("V-01", "Rate plan code is required.", "error", "General"));
  } else if (duplicate) {
    errors.push(issue("V-01", `Code "${code}" is already used by another plan.`, "error", "General"));
  }
  ...
  if (plan.effectiveFrom && plan.effectiveTo && plan.effectiveFrom > plan.effectiveTo) {
    errors.push(issue("V-06", "Effective from must be on or before effective to.", "error", "Restrictions"));
  }
  ...
  if (plan.category === "corporate" && (!plan.corporateAccountIds || plan.corporateAccountIds.length === 0)) {
    warnings.push(
      issue(
        "V-09",
  ...
  if (meal && meal.status === "Inactive") {
    warnings.push(
      issue("V-10", `Meal plan ${meal.code} is inactive.`, "warning", "Room & meal"),
    );
  }
