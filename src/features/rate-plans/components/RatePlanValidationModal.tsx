...
export function RatePlanValidationModal({
  ...
  const canPublishWithWarnings = mode === "publish" && result.canPublish && result.warnings.length > 0;
    ...
    <AlertDialog open={open} onOpenChange={onOpenChange}>
      <AlertDialogContent className="max-w-lg">
        ...
        <div className="max-h-[320px] space-y-4 overflow-y-auto py-1">
          <IssueList title="Blocking errors" issues={result.errors} tone="error" onGoToTab={onGoToTab} />
          <IssueList title="Warnings" issues={result.warnings} tone="warning" onGoToTab={onGoToTab} />
          {!blocking && result.warnings.length === 0 && (
            <div className="rounded-md border border-success/30 bg-success/10 px-3 py-2 text-[12px] text-text-primary">
              Validation checklist complete. Publish will activate the plan and create a version snapshot.
            </div>
          )}
        </div>
        ...
        <AlertDialogFooter>
          ...
          {mode === "publish" && result.canPublish && result.warnings.length === 0 && onPublishAnyway && (
