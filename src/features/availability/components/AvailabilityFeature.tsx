...
export function AvailabilityFeature() {
  ...
  const [syncJobs, setSyncJobs] = useState<SuAvailabilitySyncJob[]>(() => listSuAvailabilitySyncJobs());
    ...
    >
      <div>
        ...
        <div className="space-y-6 p-6">
          ...
          <Card>
            <CardHeader title="Availability calendar" hint="Click a cell to edit status and restrictions" />
            <div className="overflow-x-auto p-4 sm:p-5">
              <div className="min-w-[900px]">
                <div
                  className="grid gap-px bg-border-subtle"
                  style={{ gridTemplateColumns: `160px repeat(${dates.length}, minmax(56px, 1fr))` }}
                >
                  ...
                  {filteredRoomTypes.map((rt) => (
                    <div key={rt.id} className="contents">
                      ...
                      {dates.map((date) => {
                        ...
                        if (!cell) {
                          return (
                            <div key={date} className="bg-surface px-1 py-2 text-center text-[11px] text-text-disabled">
                              —
                            </div>
                          );
                        }
                        ...
                        return (
                          <div key={date} className="bg-surface px-1 py-2">
                            ...
                            >
                              ...
                              <div className="mt-0.5 truncate text-[9px] opacity-80">{cellSummary(cell)}</div>
                            </button>
                          </div>
                        );
                      })}
          ...
          {syncJobs.length > 0 ? (
            <Card>
              <CardHeader title="SU sync jobs" hint="Recent availability pushes to channel manager" />
