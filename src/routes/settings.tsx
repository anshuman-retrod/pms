...
function SettingsPage() {
  return (
    <div>
      <PageHeader eyebrow="Administration" title="System Settings" description="Account, security, and integrations." />
        ...
        <Card>
          <ul className="p-2">
            {["Account", "Security", "Notifications", "Integrations", "API & Webhooks", "Billing"].map((s, i) => (
              <li key={s}>
                <button className={`w-full rounded-md px-3 py-2 text-left text-[13px] ${i === 3 ? "bg-primary-tint text-primary-pressed font-medium" : "text-text-secondary hover:bg-surface-2 hover:text-text-primary"}`}>{s}</button>
              </li>
            ))}
          </ul>
        </Card>
            ...
            {integrations.map((it) => (
              <li key={it.name} className="flex items-center justify-between px-5 py-4">
                <div className="flex items-center gap-3">
                  <div className="flex h-9 w-9 items-center justify-center rounded-md bg-surface-2 font-display text-[14px] font-semibold text-text-primary">{it.name[0]}</div>
                  <div>
