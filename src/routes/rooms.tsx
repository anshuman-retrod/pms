...
function RoomsPage() {
  return (
    <div>
      <PageHeader eyebrow="Administration" title="Rooms & Inventory" description="Room type configuration, base rates, and live availability." actions={<Button size="sm"><Plus className="h-3.5 w-3.5" />Add room type</Button>} />
            ...
            <tbody>
              {types.map((t) => (
                <tr key={t.name} className="border-b border-border-subtle hover:bg-surface-2/50">
                  ...
                  <td className="px-4 py-3"><StatusBadge tone="success">Selling</StatusBadge></td>
                  <td className="px-4 py-3 text-right"><a className="text-[12px] font-medium text-primary">Configure</a></td>
                </tr>
              ))}
            </tbody>
          </table>
